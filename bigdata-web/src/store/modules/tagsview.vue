...
const tagsview ={
    ...
    mutations:{//这
        ADD_VISITED_VIEWS:(state,view)=>{
            ...
            let path = view.path || view.name;
            if(!state.visitedviews[path]){
                state.visitedviews[path]=view
            }
        },
        DEL_VISITED_VIEWS:(state,path)=>{//关闭页签--删除路由数据的方法
            delete state.visitedviews[path]
        },
    },
