<template>
    <div>
            ...
            <Poptip trigger="hover" placement="right" width="600">
                ...
                <div slot="content">
                    ...
                    <div style="text-align: right;margin:10px;" v-if="false">
                        <Button type="primary" @click="filterData">提交</Button>
                    </div>
                </div>
        ...
        <table-chart :loading="loading" :data="origData" :cbrTable="cbrTable" :dimension="dimension" :region="region" :city="city" ></table-chart>
    </div>
</template>
    ...
    export default {
        ...
        methods:{
            ...
            loadData(){
                this.loading = true;
                let that = this;
                this.$axios.post(this.$global.cbr_old + '/chargecar/CPortStoreCollectionCar', {}, this.$axiosConfig.normal_config)
                    .then(function (response) {
                        that.loading = false;
                        response = response.data;
                        if (response.status === 200) {
