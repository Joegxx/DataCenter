...
<script>
    ...
    function houseAvgCompute(data){
        let t = data['bids']/data['bidMerchant'];
        let y = data['yesterdayBids']/data['yesterdayBidMerchant'];
        t=t?t.toFixed(2):0;
        y=y?y.toFixed(2):0;
        return [t,y]
    }
    function referenceRateCompute(data){
        let t = data['bidMerchant']/data['totalBuyers']*100;
        let y = data['yesterdayBidMerchant']/data['yesterdayTotalBuyers']*100;
        t=t?t.toFixed(2):0;
        y=y?y.toFixed(2):0;
        return [t,y]
    }
    ...
    export default {
        ...
        data() {
            return {
                ...
                targetColumns: {
                    ...
                    'houseAvg': {name: "户均", _func: houseAvgCompute},
                    'referenceRate': {name: "参拍率", _func: referenceRateCompute},
                },
                dimensions:[
                    {
                        value: "1",
        ...
        methods: {
            loadData() {
                this.loading = true;
                let that = this;
                this.$axios.post(this.$global.cbr_old + '/auction/bidavg', this.condition, this.$axiosConfig.normal_config)
                    .then(function (response) {
                        that.loading = false;
                        response = response.data;
                        if (response.status === 200) {
